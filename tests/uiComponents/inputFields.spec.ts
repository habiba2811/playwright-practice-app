...
test.describe("Ui Components - Input Fields", () => {
  ...
  test("Input Fields", async ({ page }) => {
    ...
    const germanyaOption = page.getByRole('option', { name: 'Germany' });
    const spainOption = page.getByRole('option', { name: 'Spain' });
    await expect(germanyaOption).toBeVisible();
    ...
    const Cpanel = page.locator('.p-colorpicker-panel');
    await expect(Cpanel).toBeVisible();
    const colorBox = Cpanel.locator('[data-pc-section="color"]');
    ...
    // MultiSelect
    const multiSelectCard= page.locator('.card', { hasText: 'MultiSelect'})
    const multiSelectDropDown= multiSelectCard.locator('.p-multiselect-label')
    await multiSelectDropDown.click()
    const selectAll= multiSelectCard.locator('.p-multiselect-header.ng-star-inserted').getByRole('checkbox')
    await selectAll.click()
    await expect(selectAll).toHaveAttribute('aria-label','All items selected' )
    await selectAll.click()
    await expect(selectAll).toHaveAttribute('aria-label','All items unselected' )
    const multiSearch= multiSelectCard.locator('.p-multiselect-filter')
    await multiSearch.fill('g')
    const germanyOption= multiSelectCard.locator('.p-multiselect-list.ng-star-inserted').getByRole('option', { name: 'Germany'})
    await germanyOption.click()
    await expect(germanyOption).toBeChecked()
    await expect(multiSelectDropDown).toHaveText('Germany')
    await multiSearch.fill('c')
    const franceOption= multiSelectCard.locator('.p-multiselect-list.ng-star-inserted').getByRole('option', { name: 'France'})
    await franceOption.click()
    await expect(franceOption).toBeChecked()
    ...
    const documents = panel.locator(".p-tree-node-content", {
      hasText: "Documents",
    })
    await documents.locator('[data-pc-section="toggler"]').click()
    const work = panel.locator(".p-tree-node-content", { hasText: "Work" })
    await work.locator('[data-pc-section="toggler"]').click()
    const resume = panel.getByText("Resume.doc", { exact: true })
    await resume.click()
    await expect(page.locator(".p-treeselect-label")).toHaveText("Resume.doc")
  })
})
