...
test.describe('Ui Components - Form Layout', ()=> {
    ...
    test('input fields Vertical', async ({page})=> {
        ...
        await verticalCard.getByLabel('Email').pressSequentially('test2@test.com', {delay:500});
    ...
    test('input fields Horizontal', async ({page})=> {
        const horizontalCard = page.locator('.card', { has: page.getByText('Horizontal') });
        await horizontalCard.getByLabel('Name').fill('test');
        await horizontalCard.getByLabel('Email').fill('test@test.com');
        await expect(horizontalCard.getByLabel('Email')).toHaveValue('test@test.com')

    })

    test('input fields Vertical Grid', async ({page})=> {
        const verticalGridCard = page.locator('.card', { has: page.getByText('Vertical Grid') });
